...
def build_tools(ambiguous: bool):
    if ambiguous:
        ...
        # CRITICAL: schema field names must ALSO be uninformative in the
        # ambiguous condition — otherwise "customer_id" vs "status_filter"
        # leaks the exact disambiguating signal the description is
        # supposed to (or fails to) provide, and the test never actually
        # isolates description quality at all. (This is exactly what
        # happened in the first two attempts at this exercise: both
        # conditions scored 5/5 because informative field names let the
        # model route correctly regardless of description text.)
