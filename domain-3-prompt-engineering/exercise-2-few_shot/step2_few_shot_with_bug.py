...
this week, not blocking my shopping though"
Reasoning: A real, recurring functional bug — but the customer explicitly
says it's not blocking, and there's no dispute threat or deadline. This
stays at the level the underlying issue warrants (a real recurring defect)
rather than being downgraded by the customer's casual tone, since unlike
Example 1, this one recurred multiple times.
Urgency: MEDIUM
...
def run_condition(label, system_prompt):
    ...
    for i, ticket in enumerate(SAMPLE_TICKETS, 1):
        ...
        # print just the urgency line/level for compact comparison
        print(f"  Ticket {i}: {text[:150]}{'...' if len(text) > 150 else ''}")
    print()
