...
def _mock_response(system, user_message, tools, tool_choice):
    ...
    # Step 5 mock, Test 1: format error -> converges after one retry
    if "order_id" in system.lower() and "hash symbol" in system.lower() and not tools:
        if "validation error" in user_message.lower():
            # simulate a corrected response on retry
            return _MockResponse([_MockContentBlock(
                "text", text='{"order_id": "#4471", "urgency": "medium"}'
            )])
        # simulate a plausible first-attempt format mistake: missing the hash
        return _MockResponse([_MockContentBlock(
            "text", text='{"order_id": "4471", "urgency": "medium"}'
        )])

    # Step 5 mock, Test 2: semantic error -> generic retry does NOT
    # reliably converge to the policy-correct answer, since no new
    # information was actually given
    if "conservative" in system.lower() and "urgency" in system.lower() and "please reconsider" in user_message.lower() and not tools:
        # simulate: model second-guesses itself but lands on a DIFFERENT
        # wrong-ish answer, not the policy-correct one, since it still has
        # no actual rule to apply — just re-asked the same vague question
        return _MockResponse([_MockContentBlock(
            "text", text="On reflection, I'll rate this Medium-High — it's a legitimate billing concern, though I still don't think it rises to a full emergency."
        )])
