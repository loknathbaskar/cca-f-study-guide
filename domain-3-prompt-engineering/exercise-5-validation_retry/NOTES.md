...
# Step 5: Validation-Retry Loops — Format Errors vs. Semantic Errors

## What this demonstrates

Two separate tests, deliberately contrasted:

**Test 1 (format error — should converge)**: free-text JSON extraction
(not `tool_use`, to allow realistic format slip-ups) requiring `order_id`
to be zero-padded to exactly 6 digits (e.g. order 4471 → `"#004471"`) —
a rule with no natural basis in how the number appears in the ticket
text, chosen specifically so the model has no reason to satisfy it
without correction. (An earlier version of this test required only
`#` + digits with no padding, which the model satisfied on the first
attempt — meaning the retry loop was never actually exercised. Fixed by
picking a constraint the model won't spontaneously follow.)

**Test 2 (semantic error — should NOT reliably converge)**: the exact
VAGUE prompt + ticket 4 combination from Steps 1-2, which we already know
produces a policy-inconsistent urgency rating. The "retry" here is
deliberately generic — *"please reconsider whether this rating is
appropriate"* — with the actual policy withheld, since revealing it would
just be Step 1's explicit-criteria fix again, not a real test of what
retry alone can do.
...
```bash
export ANTHROPIC_API_KEY=sk-ant-...
python3 step5_validation_retry.py
```

**Test 1**: does it converge to valid `#NNNNNN` format within 1-2 retries?
This should work reliably — the validator has a mechanical, checkable
definition of correct.

**Test 2**: does the final urgency rating actually match HIGH (the
policy-correct answer from Steps 1-2's EXPLICIT prompt), or does it just
produce a *different* uncertain answer? Compare against the real Step 1/2
data — does generic re-asking get there, or does it just add noise?

## Why the distinction holds, precisely

A validator can only report an error it can mechanically detect. `order_id`
not matching the required pattern is checkable with a regex — the
validator doesn't need to know anything about the business, just the
pattern. "Urgency should be HIGH here" is not mechanically checkable
without re-implementing the entire threshold policy from Step 1 inside
the validator — at which point the validator itself contains the answer,
and you don't need the model to guess at that field at all; you'd compute
it directly.

**The retry loop can only pass along information the validator itself
possesses.** For format errors, the validator possesses the correct
pattern. For semantic/judgment errors, a generic validator possesses
nothing beyond "something seems off" — which is exactly as underspecified
as the original vague prompt, so retrying with it teaches the model
nothing new.
...
| Issue | Cause | Fix |
|---|---|---|
| Test 1 v1: didn't test retry at all — model satisfied the (unpadded) format on attempt 1 | Constraint was close enough to the number's natural appearance that no violation occurred | Changed to require zero-padding, a rule with no natural basis in the input |
| Test 1 v2: still didn't test retry — model satisfied the (now zero-padded) format on attempt 1 again | The zero-padding rule was placed in the **original system prompt**, so the model had it before attempt 1 — this just re-confirmed "explicit instructions work" a third time, not retry's contribution specifically | Removed the padding rule from the original prompt entirely; it's now only revealed via the retry's validation error message, creating genuine first-attempt ignorance to test against |

## Real results

**Test 1 (format error) — genuine convergence, properly isolated:**

- Attempt 1: `"4471"` — as predicted, the model naturally produced the
  ticket's own format, since `FORMAT_SYSTEM` never mentioned padding at
  all. The validator correctly caught this and fed back the specific
  rule for the first time.
- Attempt 2: `"#004471"` — correctly applied the new information from the
  retry's error message.

**This is a clean, genuine demonstration of validation-retry working as
intended**: the model had zero prior knowledge of the padding
requirement, and a single specific, mechanical error message was
sufficient to fix it in exactly one round. (This required two earlier
correction rounds to properly isolate — see the bugs/gotchas table above.
The first two attempts at this test accidentally re-confirmed "explicit
instructions work" rather than testing retry itself, since the rule was
present in the original prompt both times.)

**Test 2 (semantic/judgment error) — reproduced across two independent
real runs, with anchoring appearing even faster the second time:**

- Attempt 1: "Medium-High"
- Attempt 2 (generic "please reconsider"): still "Medium-High," now
  explicitly *"maintaining original rating... I'd stand by it"* —
  anchoring language appeared **one attempt earlier** than the first real
  run (which didn't anchor until attempt 3)
- Attempt 3: drifted to "Medium" — still wrong, and further from the
  policy-correct HIGH

**This is no longer a single observation — it's a reproduced finding
across two separate real runs.** Both times, generic reconsideration
failed to converge to the correct answer. Both times, the model
exhibited anchoring/entrenchment rather than neutral re-evaluation. The
specific final label differed between runs (this run ended at "Medium,"
the first run also ended at "Medium" after passing through "Medium-High"
then "Medium" too) but the *pattern* — failure to converge + anchoring —
held both times.

**The controlled comparison this design finally achieves**: both tests
start with an underspecified original prompt (Test 1 never mentions
padding; Test 2 never states the urgency policy). The only difference is
what the retry message contains — Test 1's retry supplies the specific
missing rule; Test 2's retry deliberately doesn't (mirroring what a
truly generic "please double-check" retry looks like in practice, with
no business logic embedded in it). Test 1 converges immediately once
given the missing information. Test 2 never receives the missing
information and never converges — instead getting worse through anchoring.
**The difference in outcome tracks the difference in what the retry
message actually contained, not some inherent property of "format vs.
semantic" errors.** A validator can only supply what it knows — for
format rules, that's a specific pattern; for policy judgment, a generic
validator has nothing specific to supply, which is the real, more precise
version of the "retry doesn't fix semantic errors" claim.

## Conclusion for Step 5

Format-level validation-retry works well **specifically because the
retry message can carry new, specific, actionable information** the
original prompt lacked — confirmed directly, not just assumed, once the
test was properly isolated from Steps 1-2's "explicit prompting works"
effect. For judgment/semantic correctness, generic retry doesn't just
fail to help — reproduced twice now, it actively entrenches a wrong
answer through anchoring, which is a stronger and more specific claim
than the original "retry is ineffective" hypothesis. The fix for
judgment errors remains Steps 1-2's approach (explicit criteria, targeted
few-shot) — information injected *before* an answer, not feedback
supplied *after* one that doesn't actually contain the missing rule.

## Design principle worth defending in an exam-style answer

**If validation-retry can't fix semantic miscalibration, what's the
actual fix?** Steps 1 and 2's fixes — explicit criteria and targeted
few-shot examples — work because they inject the missing information
*before* the model answers, rather than reacting *after* an already-wrong
answer with feedback that doesn't contain the missing information either.
Retry loops are for catching mechanical mistakes a well-specified prompt
still occasionally produces — not a substitute for actually specifying
the task correctly in the first place.

## Next step
Step 6 — the Message Batches API: cost/latency trade-offs, and what
multi-turn tool calling gives up in batch mode.
