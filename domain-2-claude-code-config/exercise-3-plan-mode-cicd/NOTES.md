...
| Documented exit-code convention (0 success / 1 generic error / 2 auth error) does not match observed behavior | A bad `ANTHROPIC_API_KEY` produced exit code **1**, not the documented **2**. **Root cause, confirmed**: the exit code alone doesn't encode failure category at all on this version — `review.json` was still written on failure with `is_error: true`, `api_error_status: 401` (the real HTTP status), `terminal_reason: "api_error"`, and `result: "Invalid API key · Fix external API key"`. stderr only carried an unrelated connectors-precedence warning, not the actual error. The exit-code-to-category table several third-party docs describe simply isn't how this version surfaces error detail — the structured JSON body is. | Script no longer branches on the specific exit code number. Instead checks `is_error` in the JSON output itself, and if true, reports `api_error_status` + `result` directly — the actual reliable source of truth for *why* something failed, present even when the process exit code doesn't clearly convey it. |
